...
* in [task 28]() with STDIN++ Launcher, you should pay attention to ```&& sEt```, ```^|PowersHElL``` and ```&&  CMd``` patterns, rather than the ```$Env:cOmSPec[4,26,``` pattern, which is already covered by Daniel's rule.
