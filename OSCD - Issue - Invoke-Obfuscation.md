...
* in [task 25]() with the STDIN+ Launcher, you should pay attention to ```cmd /c``` and ```| powershell``` patterns, rather than the ```$SHElLID[1]+$ShELlId[``` pattern, which is already covered by Daniel's rule;
* in [task 28]() with STDIN++ Launcher, you should pay attention to ```&& sEt```, ```^|PowersHElL``` and ```&&  CMd``` patterns, rather than the ```$Env:cOmSPec[4,26,25]-jOiN``` pattern, which is already covered by Daniel's rule;
